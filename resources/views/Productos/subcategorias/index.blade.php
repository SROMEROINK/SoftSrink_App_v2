...
@section('title', 'Subcategorias de Producto')

@section('content_header')
<x-header-card
    title="Subcategorias de Producto"
    buttonRoute="{{ route('producto_subcategoria.create') }}"
    buttonText="Crear Subcategoria"
    deletedRouteUrl="{{ route('producto_subcategoria.deleted') }}"
    ...
    <div class="row mt-3">
        <div class="col-md-4">
            <div class="small-box bg-info">
                <div class="inner">
                    <h3 id="total-subcategorias">{{ $totalSubcategorias }}</h3>
                    <p>Total de subcategorias</p>
                </div>
                <div class="icon">
        ...
        <div class="col-md-4">
            <div class="small-box bg-success">
                <div class="inner">
                    <h3 id="activas-subcategorias">0</h3>
                    <p>Subcategorias activas</p>
                </div>
                <div class="icon">
        ...
        <div class="col-md-4">
            <div class="small-box bg-danger">
                <div class="inner">
                    <h3 id="eliminadas-subcategorias">0</h3>
                    <p>Subcategorias eliminadas</p>
                </div>
                <div class="icon">
    ...
    <div class="card mt-3">
        <div class="card-body">
            <div class="table-responsive">
                <table id="tabla_producto_subcategoria" class="table table-bordered table-striped w-100">
                    <thead>
                        <tr>
                            <th>Categoria</th>
                        ...
                        <tr>
                            <th></th>
                            <th><input type="text" id="filtro_nombre" class="form-control form-control-sm filtro-texto" placeholder="Filtrar Subcategoria"></th>
                            <th>
...
@section('css')
    ...
    <link rel="stylesheet" href="{{ asset('vendor/adminlte/dist/css/productos/subcategorias/index.css') }}">
@stop
...
<script>
    $(document).ready(function () {
        function cargarResumen() {
            $.get("{{ route('producto_subcategoria.resumen') }}", function (data) {
                $('#total-subcategorias').text(data.total);
                $('#activas-subcategorias').text(data.activos);
                $('#eliminadas-subcategorias').text(data.eliminados);
            });
        }

        function cargarFiltros() {
            $.get("{{ route('producto_subcategoria.filters') }}", function (data) {
            ...
            columns: [
                ...
                { data: 'updated_at', name: 'updated_at' },
                {
                    ...
                    render: function (data) {
                        return `
                            <a href="{{ url('producto_subcategoria') }}/${data}" class="btn btn-info btn-sm">Ver</a>
                            <a href="{{ url('producto_subcategoria') }}/${data}/edit" class="btn btn-primary btn-sm">Editar</a>
        ...
        $(document).on('click', '.btn-eliminar', function () {
            ...
            SwalUtils.confirmDelete('La subcategoria sera enviada a eliminados.').then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ url('producto_subcategoria') }}/${id}`,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
