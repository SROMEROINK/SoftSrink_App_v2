...
@section('title', 'Crear Grupo de Conjuntos')

@section('content_header')
    <h1>Crear Grupo de Conjuntos</h1>
@stop
...
@section('content')
    ...
    <div class="card mt-3">
        <div class="card-header">
            <h3 class="card-title">Nuevo grupo de conjuntos</h3>
        </div>

        <form method="POST"
              action="{{ route('producto_grupo_conjuntos.store') }}"
              data-ajax="true"
            ...
            <div class="card-body">
                @if($ultimoGrupo)
                    <div class="alert alert-info">
                        Ultimo ID registrado: <strong>{{ $ultimoGrupo->Id_GrupoConjuntos }}</strong> -
                        Ultimo grupo: <strong>{{ $ultimoGrupo->Nombre_GrupoConjuntos }}</strong>
                    </div>
                @endif

                <div class="form-group">
                    <label for="Nombre_GrupoConjuntos">Nombre del Grupo</label>
                    <input type="text" name="Nombre_GrupoConjuntos" id="Nombre_GrupoConjuntos" class="form-control" maxlength="100" required>
                </div>
...
@section('css')
    ...
    <link rel="stylesheet" href="{{ asset('vendor/adminlte/dist/css/productos/grupos_conjuntos/create.css') }}">
@stop
