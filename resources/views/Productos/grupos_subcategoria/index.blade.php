...
@section('title', 'Grupos de Subcategoria')

@section('content_header')
<x-header-card
    title="Grupos de Subcategoria"
    buttonRoute="{{ route('producto_grupo_subcategoria.create') }}"
    buttonText="Crear Grupo"
    deletedRouteUrl="{{ route('producto_grupo_subcategoria.deleted') }}"
    ...
    <div class="card mt-3">
        <div class="card-body">
            <div class="table-responsive">
                <table id="tabla_grupo_subcategoria" class="table table-bordered table-striped w-100">
                    <thead>
                        <tr>
                            <th>Subcategoria</th>
                            <th>Nombre Grupo</th>
...
@section('css')
    ...
    <link rel="stylesheet" href="{{ asset('vendor/adminlte/dist/css/productos/grupos_subcategoria/index.css') }}">
@stop
...
<script>
    $(document).ready(function () {
        function cargarResumen() {
            $.get("{{ route('producto_grupo_subcategoria.resumen') }}", function (data) {
        ...
        function cargarFiltros() {
            $.get("{{ route('producto_grupo_subcategoria.filters') }}", function (data) {
        ...
        const table = $('#tabla_grupo_subcategoria').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('producto_grupo_subcategoria.data') }}",
                data: function (d) {
            ...
            columns: [
                ...
                {
                    data: 'Id_GrupoSubCategoria',
                    name: 'acciones',
                    orderable: false,
                    searchable: false,
                    render: function (data) {
                        return `
                            <a href="{{ url('producto_grupo_subcategoria') }}/${data}" class="btn btn-info btn-sm">Ver</a>
                            <a href="{{ url('producto_grupo_subcategoria') }}/${data}/edit" class="btn btn-primary btn-sm">Editar</a>
        ...
        $(document).on('click', '.btn-eliminar', function () {
            ...
            SwalUtils.confirmDelete('El grupo sera enviado a eliminados.').then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ url('producto_grupo_subcategoria') }}/${id}`,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
